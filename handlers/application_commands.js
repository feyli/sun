...
// Grab the SlashCommandBuilder#toJSON() output of each command's data for deployment
    for (const folder of categoryFolders) {
        for (const file of fs.readdirSync(`${commandsPath}/${folder}`).filter(file => file.endsWith('.js'))) {
...
// and deploy your commands!
    try {
        ...
        let data = await rest.put(
            Routes.applicationCommands(config.client.id),
            {
                body: client.commands.filter(
                    (command) => command.type !== 'button' && !command.guild_id).map((command) => command.command_data),
            },
        );
        ...
        const guilds = client.commands.filter(
            (command) => command.type !== 'button' && command.guild_id).map((command) => command.guild_id);
        for (const guild of guilds) {
            const guildCommands = client.commands.filter(
                (command) => command.type !== 'button' && command.guild_id === guild).map((command) => command.command_data);
