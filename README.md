...
Bun loads the `.env` file automatically. The PostgreSQL settings have no defaults: the bot fails on startup if any of them is missing, so no deployment details live in the repository.
